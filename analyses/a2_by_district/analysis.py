...
def _readme_section(results):
    ...
    for religion, rows in results.items():
        ...
        for row in rows:
            ...
            lines.append(
                f"| {district_label} | {proportion_national} | {row['2012']:,} | {row['2024']:,} | {row['change']:+,}{triangle(row['change'])} | {_format_share(row['proportion_2012'])} | {_format_share(row['proportion_2024'])} | {_format_pp(row['proportion_change'])}pp{triangle(_rounded_pp(row['proportion_change']))} |"
            )
