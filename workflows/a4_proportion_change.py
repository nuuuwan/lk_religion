...
def run():
    ...
    for code in db_dsd_2012:
        ...
        eligible = [
            r for r in RELIGIONS
            if max(db_dsd_2012[code].get(r, 0), db_dsd_2024[code].get(r, 0)) >= MIN_ABSOLUTE
            and max(db_dsd_2012[code].get(r, 0), db_dsd_2024[code].get(r, 0)) >= national2024[r] * MIN_NATIONAL_SHARE
        ]
        if not eligible:
            continue
    ...
    with open(os.path.join(DATA_DIR, "proportion_change_analysis.json"), "w") as f:
        json.dump({"by_district": district_rows, "by_dsd": dsd_rows}, f, indent=2)

    print(f"\n  By District:")
    print(f"  {'District':<16} {'Religion':<16} {'2012':>8} {'2024':>8} {'Change':>10}")
    print("  " + "-" * 62)
    for row in district_rows:
        print(f"  {row['district']:<16} {row['religion']:<16} {row['proportion_2012']:>8.1%} {row['proportion_2024']:>8.1%} {row['change']:>+10.1%}")

    print(f"\n  By DSD (top {TOP_DSD}, excl. altered/new/removed):")
    print(f"  {'DSD':<22} {'District':<14} {'Religion':<16} {'2012':>8} {'2024':>8} {'Change':>10}")
    print("  " + "-" * 82)
    for row in dsd_rows[:TOP_DSD]:
        print(f"  {row['dsd']:<22} {row['district']:<14} {row['religion']:<16} {row['proportion_2012']:>8.1%} {row['proportion_2024']:>8.1%} {row['change']:>+10.1%}")
