...
from lk_religion.common.ReligionUtils import RELIGIONS, RELIGION_LABELS, shares, triangle

ROOT_DIR = Path(__file__).resolve().parents[3]
ANALYSIS_DIR = ROOT_DIR / 'analyses' / 'a3_proportion_change'
README_PATH = ANALYSIS_DIR / 'README.md'
CHART_PATH = ANALYSIS_DIR / 'chart.png'
A2_ANALYSIS_DIR = ROOT_DIR / 'analyses' / 'a2_by_district'
...
def run():
    ...
    for religion in RELIGIONS:
        religion_rows = [row for row in district_rows if row['religion'] == religion]
        if not religion_rows:
            continue
        print(f'\n  {religion}:')
        print(f"  {'District':<16} {'2012':>8} {'2024':>8} {'Change':>10}")
        print('  ' + '-' * 48)
    ...
    return write_markdown(README_PATH, _readme_section(district_rows))
