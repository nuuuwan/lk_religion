...
def _load_region_data(config):
    if config.data_loader is not None:
        db2012, db2024 = config.data_loader(config)
    ...
    for religion, rows in results.items():
        ...
        for row in display_rows:
            ...
            print(
                ...
                f"Δ={row['change']:+,} share12={format_share(row['proportion_2012'])} "
                f"share24={format_share(row['proportion_2024'])} "
                f"Δpp={format_pp(row['proportion_change'])}"
            )
...
def _write_readme(config, results):
    ...
    for religion, rows in results.items():
        ...
        if config.table_min_abs_pp is not None:
            filtered_rows = [
                row
                for row in rows
                if abs(rounded_pp(row['proportion_change'])) > config.table_min_abs_pp
            ]
    ...
    return write_markdown(config.readme_path, '\n'.join(lines))
    ...
    for ax, religion in zip(axes.flat, RELIGIONS):
        ...
        if config.label_top_n != 0:
            ...
            for _, row in plot_gdf.iterrows():
                ...
                label = (
                    f"{row[config.name_key]}\n{format_pp(proportion_change)} pp"
